...
export class LinearStory extends CFEFile {
	static CLASS_DEPTH = 1;

	fileType = 'Linear Story';
	...
	static override async CreateNewFileForLayer(data: FileCreationData): Promise<LinearStory> {
		const newStoryFile = <LinearStory> (await super.CreateNewFileForLayer(data));
		newStoryFile.currentPageIndex = 0;
		newStoryFile.pages = [];
		newStoryFile.characters = [];
		newStoryFile.font = 'Inter';
		newStoryFile.isVertical = false;
		return newStoryFile;
	}
	...
	private async LoadStoryUI(snv: SourceAndVault, mainDiv: HTMLDivElement) {
		...
		mainDiv.createEl('p', { text: 'Do vertical text:' } );
		const isVerticalInput = mainDiv.createEl('input', { type: 'checkbox' } );
		isVerticalInput.checked = this.isVertical;
		isVerticalInput.onchange = () => {
			this.isVertical = isVerticalInput.checked;
		}
		mainDiv.createEl('p', { text: 'Font:' } );
		const fontInput = mainDiv.createEl('input', { type: 'text' } );
		fontInput.value = this.font;
		fontInput.onchange = () => {
			this.font = fontInput.value;
		}
	...
	private async LoadDialogueLines(snv: SourceAndVault, linesDiv: HTMLDivElement) {
		...
		if (this.isVertical) {
			this.LoadVerticalLines(snv, linesDiv);
		} else {
			this.LoadHorizontalLines(snv, linesDiv);
		}
	}

	private async LoadVerticalLines(snv: SourceAndVault, linesDiv: HTMLDivElement) {
	...
	private async LoadHorizontalLines(snv: SourceAndVault, linesDiv: HTMLDivElement) {
		...
		for (let i = 0; i < this.pages[this.currentPageIndex].lines.length; i++) {
			...
			indexElement.style.color = currentSpeaker.color;
			indexElement.style.fontFamily = this.font;
			const charDropdown = lineDiv.createEl('select');
			for (let i = 0; i < this.characters.length; i++) {
				const currentChar = this.characters[i];
				const currentOption = charDropdown.createEl('option', { text: currentChar.name, value: '' + i } );
				currentOption.style.color = currentChar.color;
				currentOption.style.backgroundColor = currentChar.backgroundColor;
			}
			charDropdown.value = '' + currentLine.speakerIndex;
			charDropdown.style.color = currentSpeaker.color;
			charDropdown.style.backgroundColor = currentSpeaker.backgroundColor;
			charDropdown.style.fontFamily = this.font;
			const lineInput = lineDiv.createEl('textarea');
			...
			charDropdown.onchange = async () => {
				const charIndex = parseInt(charDropdown.value);
				this.pages[this.currentPageIndex].lines[currentIndex].speakerIndex = charIndex;
				const newSpeakerColor = this.characters[charIndex].color;
				const newBackgroundColor = this.characters[currentIndex].backgroundColor;
				charDropdown.style.color = newSpeakerColor;
				charDropdown.style.backgroundColor = newBackgroundColor;
				lineInput.style.color = newSpeakerColor;
				lineInput.style.backgroundColor = newBackgroundColor;
		...
		const addButton = linesDiv.createEl('button', { text: '+' } );
		addButton.onclick = async () => {
			this.pages[this.currentPageIndex].lines.push(new DialogueLine(0, ''));
			await this.Save(snv);
			await this.LoadDialogueLines(snv, linesDiv);
